...
public class HeavyBag<T> extends AbstractCollection<T> implements Serializable {
	...
	public String toString() {
		...
		for (T element : uniqueElements()) {
			for (int i = 0; i < bag.get(element); i++) {
				answer += element;
			}
		}
		return answer;
	...
	public boolean equals(Object o) {
		...
		HeavyBag<T> oBag = (HeavyBag<T>) o;
		if (this.containsAll(oBag)) {
			int total = 0;
			for (T element : this.uniqueElements()) {
				if (bag.get(element) == oBag.bag.get(element)) {
					total++;
				}
			}
			if (total == uniqueElements().size()) {
				return true;
			}
		}
		return false;
	...
	public Iterator<T> iterator() {
		return new Iterator<T>() {
			...
			T[] bagArr = (T[]) uniqueElements().toArray();
			int index = 0;
			...
			public T next() {
				if (elementCount > 0) {
					...
					returnElement = bagArr[index];
					elementCount = bag.get(bagArr[index]) - 1;
					index++;
				}
				pos++;
			...
			@Override
			public void remove() {
				HeavyBag.this.remove(returnElement);
			}
		};
	}
